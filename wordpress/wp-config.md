...
```
$table\_prefix = 'r235\_'; // Only numbers, letters, and underscores please!
```

### [WP\_SITEURL](#wp-siteurl)
...
```
define( 'WP\_SITEURL', 'http://example.com/wordpress' );
```

Dynamically set WP\_SITEURL based on $\_SERVER\[‘HTTP\_HOST’\]

```
define( 'WP\_SITEURL', 'http://' . $\_SERVER\['HTTP\_HOST'\] . '/path/to/wordpress' );
```
...
```
define( 'WP\_SITEURL', 'http://' . $\_SERVER\['SERVER\_NAME'\] . '/path/to/wordpress' );
```
...
```
define( 'WP\_HOME', 'http://example.com/wordpress' );
```
...
```
define( 'WP\_HOME', 'http://' . $\_SERVER\['HTTP\_HOST'\] . '/path/to/wordpress' );
```
...
```
define( 'WP\_CONTENT\_DIR', dirname(\_\_FILE\_\_) . '/blog/wp-content' );
```

Set WP\_CONTENT\_URL to the full **URL** of this directory (no trailing slash), e.g.

```
define( 'WP\_CONTENT\_URL', 'http://example/blog/wp-content' );
```
...
```
define( 'AUTOSAVE\_INTERVAL', 160 ); // Seconds
```
...
```
define( 'COOKIE\_DOMAIN', 'www.example.com' );
```
...
```
define( 'WP\_DISABLE\_FATAL\_ERROR\_HANDLER', true );   // 5.2 and later define( 'WP\_DEBUG', true );
...
```
define( 'WP\_DISABLE\_FATAL\_ERROR\_HANDLER', true );   // 5.2 and later
...
### [WP\_ENVIRONMENT\_TYPE](#wp-environment-type)
...
```
define( 'WP\_ENVIRONMENT\_TYPE', 'staging' );
```
...
```
error\_reporting = 4339
display\_errors = Off
display\_startup\_errors = Off
log\_errors = On
error\_log = /home/example.com/logs/php\_error.log
...
@ini\_set( 'display\_errors', 'Off' );
@ini\_set( 'error\_log', '/home/example.com/logs/php\_error.log' );
...
/\*\*
 \* This will log all errors notices and warnings to a file called debug.log in
 \* wp-content only when WP\_DEBUG is true. if Apache does not have write permission,
 \* you may need to create the file first and set the appropriate permissions (i.e. use 666).
 \*/
define( 'WP\_DEBUG', true ); // Or false
if ( WP\_DEBUG ) {
    define( 'WP\_DEBUG\_LOG', true );
    define( 'WP\_DEBUG\_DISPLAY', false );
    @ini\_set( 'display\_errors', 0 );
...
@ini\_set( 'log\_errors', 'On' );
@ini\_set( 'display\_errors', 'Off' );
define( 'WP\_DISABLE\_FATAL\_ERROR\_HANDLER', false );   // 5.2 and later
...
```
define( 'WP\_MAX\_MEMORY\_LIMIT', '128M' );
```
...
define( 'WP\_LANG\_DIR', dirname(\_\_FILE\_\_) . 'wordpress/languages' );
```
...
```
define( 'FS\_CHMOD\_DIR', ( 0755 & ~ umask() ) );
define( 'FS\_CHMOD\_FILE', ( 0644 & ~ umask() ) );
```

Example to provide setgid:

```
define( 'FS\_CHMOD\_DIR', ( 02755 & ~umask() ) );
```
...
```
define( 'FS\_METHOD', 'ftpext' );
define( 'FTP\_BASE', '/path/to/wordpress/' );
define( 'FTP\_CONTENT\_DIR', '/path/to/wordpress/wp-content/' );
define( 'FTP\_PLUGIN\_DIR ', '/path/to/wordpress/wp-content/plugins/' );
define( 'FTP\_PUBKEY', '/home/username/.ssh/id\_rsa.pub' );
define( 'FTP\_PRIKEY', '/home/username/.ssh/id\_rsa' );
define( 'FTP\_USER', 'username' );
define( 'FTP\_PASS', 'password' );
define( 'FTP\_HOST', 'ftp.example.org' );
define( 'FTP\_SSL', false );
```
...
```
define( 'WP\_CRON\_LOCK\_TIMEOUT', 60 );
```
...
```
define( 'COOKIEPATH', preg\_replace( '|https?://\[^/\]+|i', '', get\_option( 'home' ) . '/' ) );
define( 'SITECOOKIEPATH', preg\_replace( '|https?://\[^/\]+|i', '', get\_option( 'siteurl' ) . '/' ) );
define( 'ADMIN\_COOKIE\_PATH', SITECOOKIEPATH . 'wp-admin' );
define( 'PLUGINS\_COOKIE\_PATH', preg\_replace( '|https?://\[^/\]+|i', '', WP\_PLUGIN\_URL ) );
define( 'TEMPLATEPATH', get\_template\_directory() );
define( 'STYLESHEETPATH', get\_stylesheet\_directory() );
```
...
```
define( 'EMPTY\_TRASH\_DAYS', 30 ); // 30 days
```

To disable trash set the number of days to zero.

```
define( 'EMPTY\_TRASH\_DAYS', 0 ); // Zero days
```
...
### [**DO\_NOT\_UPGRADE\_GLOBAL\_TABLES**](#do-not-upgrade-global-tables)
...
```
define( 'DO\_NOT\_UPGRADE\_GLOBAL\_TABLES', true );
```
...
```
print\_r( @get\_defined\_constants() );
```
...
```
define( 'WP\_HTTP\_BLOCK\_EXTERNAL', true );
define( 'WP\_ACCESSIBLE\_HOSTS', 'api.wordpress.org,\*.github.com' );
```
...
```
define( 'WP\_AUTO\_UPDATE\_CORE', false );
```
...
```
define( 'WP\_AUTO\_UPDATE\_CORE', 'minor' );
```
...
```
define( 'IMAGE\_EDIT\_OVERWRITE', true );
```
